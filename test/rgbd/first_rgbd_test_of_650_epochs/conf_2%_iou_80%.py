...
def main():
    try:
        model = YOLO('/home/jacobo/dataset/new_train_tiff/my_yolo_train/mines_rgbd_train/weights/best.pt')

        print("Starting YOLO model evaluation on the TEST split...")
        print("-" * 50)

        metrics = model.val(
            data='/home/jacobo/dataset/test_rgbd/data.yaml',
            imgsz=800,
            batch=16,
            device='0',
            workers=8,
            ...
            name='eval_on_rgbd_iou_80%_conf_2%',
            verbose = True,
            rect=False,
            single_cls=False,
            iou=0.8,
